...
local function entry(st, job)
	...
	local R = { parent = ratio[1], current = ratio[2], preview = ratio[3] }
	...
	elseif act == "max" then
		...
		st[to] = max
	elseif act == "ratio" and to:match("^%d+,%d+,%d+$") then
		st.ratio[to] = not st.ratio[to]
		if st.ratio[to] then
			local p, c, v = string.match(job.args[1] or "", "(%d+),(%d+),(%d+)")
			st.parent = tonumber(p)
			st.current = tonumber(c)
			st.preview = tonumber(v)
		else
			st.parent = R.parent
